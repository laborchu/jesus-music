...
export function loadCompositeTasks(seedTasksFile: string, projectTasksFile: string): void {
  ...
  try {
    let seedTasksStr = readFileSync(seedTasksFile).toString();
    seedTasks = JSON.parse(seedTasksStr);
    let projectTasksStr = readFileSync(projectTasksFile).toString();
    projectTasks = JSON.parse(projectTasksStr);
  } catch (e) {
